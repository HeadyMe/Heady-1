...
export class SystemIntegrator extends EventEmitter {
    ...
    async initialize() {
        ...
        try {
            ...
            this.orchestrator = new NodeOrchestrator(this.config.deterministicSeed, { type: 'capability-match' });
            ...
            this.router = new OptimizedTaskRouter(this.orchestrator, this.monitor, this.workflowEngine);
    ...
    // Register nodes from registry
    async registerNodesFromRegistry() {
        if (!this.nodeRegistry?.nodes)
            return;
    ...
    handleRecoveryRequest(msg) {
        ...
        if (recoveryNodes.length > 0) {
            ...
            const response = this.protocol.createMessage('system', source, 'RECOVERY_RESPONSE', {
...
export function createSystemConfig(databaseUrl, overrides = {}) {
    return {
        databaseUrl,
        ...overrides,
    };
