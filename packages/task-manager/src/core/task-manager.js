...
export class TaskManager extends EventEmitter {
    ...
    setupEventHandlers() {
        ...
        this.taskQueue.on('task:created', async (event) => {
            await this.handleTaskCreated(event);
        });
        this.taskQueue.on('task:queued', async (event) => {
            await this.handleTaskQueued(event);
        });
        this.taskQueue.on('task:started', async (event) => {
            await this.handleTaskStarted(event);
        });
        this.taskQueue.on('task:progress', async (event) => {
            await this.handleTaskProgress(event);
        });
        this.taskQueue.on('task:completed', async (event) => {
            await this.handleTaskCompleted(event);
        });
        this.taskQueue.on('task:failed', async (event) => {
            await this.handleTaskFailed(event);
        });
        this.taskQueue.on('task:cancelled', async (event) => {
        ...
        this.registerExecutor({
            ...
            async execute(payload, task) {
                ...
                for (let i = 0; i < payload.length; i++) {
                    results.push(await this.processBatchItem(payload[i]));
    ...
    registerExecutor(executor) {
        ...
        // Register with queue
        this.taskQueue.registerExecutor(executor.type, async (payload, task, context) => {
    ...
    async createChildTask(parentId, taskData) {
    ...
    async handleTaskCompleted(event) {
        ...
        if (this.metricsCollector) {
            ...
            if (task) {
                this.metricsCollector.recordTaskCompleted(event.taskId, task.type, task.startedAt ? Date.now() - task.startedAt.getTime() : 0);
            }
        }
    }
    ...
    startMetricsCollection() {
        ...
        this.metricsInterval = setInterval(async () => {
            ...
            if (this.websocketServer) {
                this.websocketServer.emitMetrics(metrics);
            }
