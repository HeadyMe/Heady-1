...
import { NodeStatus } from './node-orchestrator.js';
export class ArenaManager extends EventEmitter {
    ...
    async joinMatch(matchId, nodeId) {
        const match = this.matches.get(matchId);
        if (!match)
            return false;
        if (match.status !== 'pending')
            return false;
        if (match.players.includes(nodeId))
            return true;
    ...
    async submitSolution(matchId, nodeId, content) {
        const match = this.matches.get(matchId);
    ...
    async concludeRound(matchId) {
        ...
        if (currentRound.submissions.length > 0) {
            ...
            const sortedSubmissions = [...currentRound.submissions].sort((a, b) => (b.metrics?.qualityScore || 0) - (a.metrics?.qualityScore || 0));
    ...
    mergeSubmissionsInMemory(submissions) {
        if (submissions.length === 0)
            return '';
        if (submissions.length === 1)
            return submissions[0].content;
    ...
    emitEvent(type, matchId, payload) {
        const event = {
            type,
            matchId,
            payload,
...
//# sourceMappingURL=arena-manager.js.map