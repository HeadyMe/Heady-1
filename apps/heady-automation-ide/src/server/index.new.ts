...
import { initializeTaskSystem, stopTaskSystem, taskManager } from './services/task-system.js';
...
app.get('/api/queue/status', rateLimits.standard, asyncHandler(async (req, res) => {
  const status = await taskManager.getQueueStatus();
  res.json(status);
}));
...
app.post('/api/task/batch', rateLimits.mcpTasks, asyncHandler(async (req, res) => {
  ...
  // Create a batch task
  const batchTask = await taskManager.createTask({
    type: 'batch',
...
httpServer.listen(PORT, async () => {
  ...
  // Start monitoring
  monitoringService.startMonitoring(5000);
  logger.info('Real-time monitoring started', { intervalMs: 5000 });
});
